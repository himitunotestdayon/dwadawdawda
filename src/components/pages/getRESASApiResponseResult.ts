export const getRESASApiResponseResult = (res: any) =>{
    if(res.data === '400' || res.data.result === null){
        ...
        return {code:200, message:"URLに該当するAPIがありません。アドレスに誤りはないかバージョンアップにより廃止されていないかご確認ください。", result:undefined}
        ...
        */
        return {code:200, message:"APIキーが無効、またはURLが間違っている可能性があります。必要があれば管理者にお問い合わせください。", result:undefined}
    }else if("message" in res.data && res.message != null){
        return {code:200, message:"APIキーの利用可能上限を超えました。期間をおいてまたお試しください", result:undefined}
    }else{
        return {code:200, message:"成功", result:res.data.result}
    }
}